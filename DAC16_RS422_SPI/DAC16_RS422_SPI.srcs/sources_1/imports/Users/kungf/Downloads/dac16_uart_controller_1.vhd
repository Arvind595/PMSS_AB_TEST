...
--   Byte1 = 0x01                 command: write DAC code
--   Byte2 = channel (0-15)
--   Byte3 = DAC code[15:8]       MSB - computed externally (e.g. via
--   Byte4 = DAC code[7:0]        LSB   dac_voltage_calculator.html)
--   Byte5 = 0x00                 unused
--   Byte6 = 0x00                 unused
--   Byte7 = 0x00                 reserved
--
-- The 16-bit code is taken directly from Byte3/Byte4 and written to the
-- selected channel's DAC register as-is - no voltage-to-code conversion
-- happens in the FPGA. Any 16-bit value is valid (the DAC register itself
-- has no invalid codes), so there's nothing to clamp or range-check here;
-- get the code right on the host side using the calculator tool.
--
-- On a valid command (Byte0=0x55, Byte1=0x01, Byte2 in 0..15):
--   - Byte3/Byte4 are written to the selected channel's DAC register
...
--   instead of replying) for either of those two cases. Byte5/Byte6 aren't
--   validated as 0x00 (same as Byte7) - a host sending garbage there won't
--   be rejected, since they're simply never read.
--
-- Register addresses (0x18/0x19/0x2A) are VERIFIED against the AD3551R
-- datasheet (Rev. A) - see the Registers tab of dac_voltage_calculator.html.
-- The power-on staircase defaults (CH_DAC_CODE below) were computed against
-- the datasheet-exact transfer function for CH0_OUTPUT_RANGE = 0x04
-- (code 0x0000 = -10.382V, code 0xFFFF = +10.380V per Table 8 - not an
-- idealized +/-10.000V); that only matters for the boot-time defaults now,
-- since runtime writes take a code directly and never touch that formula.
--------------------------------------------------------------------------------
                ...
                case app_state is
                    ...
                    when ST_VALIDATE =>
                        ...
                        if is_valid_v then
                            ...
                            -- code taken directly from the packet: Byte3 = MSB, Byte4 = LSB
                            computed_code <= rx_latched(3) & rx_latched(4);
                        end if;
