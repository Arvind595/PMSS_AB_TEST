...
--
-- Expected DAC codes below use the corrected (datasheet-exact) transfer
-- function: code 0x0000 = -10.382V, code 0xFFFF = +10.380V (Table 8), not
-- an idealized +/-10V - see dac16_uart_controller.vhd's uv_to_code comment.
--
...
begin
    ...
    begin
        ...
        ------------------------------------------------------------------
        -- Test 1: valid write, CH3, +5.000V -> code 0xBDA9
        ------------------------------------------------------------------
        p := (x"55", x"01", x"03", x"00", x"4C", x"4B", x"40", x"00");
        run_case("T1 CH3 +5.000V", p, p, true, 3, x"BDA9", fail_count);

        ------------------------------------------------------------------
        -- Test 2: valid write, CH7, -5.000V -> code 0x425C
        ------------------------------------------------------------------
        p := (x"55", x"01", x"07", x"FF", x"B3", x"B4", x"C0", x"00");
        run_case("T2 CH7 -5.000V", p, p, true, 7, x"425C", fail_count);

        ------------------------------------------------------------------
        -- Test 3: invalid command byte (0x02, not 0x01), CH5
        -- expect: reply byte1 = 0xFF, everything else echoed, no SPI write
        ------------------------------------------------------------------
        p := (x"55", x"02", x"05", x"00", x"13", x"12", x"D0", x"00");
        r := p; r(1) := x"FF";
        ...
        ------------------------------------------------------------------
        p := (x"AA", x"01", x"02", x"00", x"4C", x"4B", x"40", x"00");
        r := p; r(1) := x"FF";
        ...
        ------------------------------------------------------------------
        p := (x"55", x"01", x"14", x"00", x"4C", x"4B", x"40", x"00"); -- 0x14 = 20
        r := p; r(1) := x"FF";
        run_case("T5 out-of-range channel", p, r, false, 0, x"0000", fail_count);

        ------------------------------------------------------------------
        -- Test 6: clamp positive overrange, CH10, +15.000V -> clamps to +10V (0xFB50)
        -- command itself is well-formed, so it's still acknowledged as valid
        ------------------------------------------------------------------
        p := (x"55", x"01", x"0A", x"00", x"E4", x"E1", x"C0", x"00");
        run_case("T6 CH10 +15V clamp", p, p, true, 10, x"FB50", fail_count);

        ------------------------------------------------------------------
        -- Test 7: clamp negative overrange, CH10, -15.000V -> clamps to -10V (0x04B6)
        ------------------------------------------------------------------
        p := (x"55", x"01", x"0A", x"FF", x"1B", x"1E", x"40", x"00");
        run_case("T7 CH10 -15V clamp", p, p, true, 10, x"04B6", fail_count);

        ------------------------------------------------------------------
        -- Test 8: exact positive boundary, CH12, +10.000V -> 0xFB50
        ------------------------------------------------------------------
        p := (x"55", x"01", x"0C", x"00", x"98", x"96", x"80", x"00");
        run_case("T8 CH12 +10.000V boundary", p, p, true, 12, x"FB50", fail_count);

        ------------------------------------------------------------------
        -- Test 9: exact negative boundary, CH12, -10.000V -> 0x04B6
        ------------------------------------------------------------------
        p := (x"55", x"01", x"0C", x"FF", x"67", x"69", x"80", x"00");
        run_case("T9 CH12 -10.000V boundary", p, p, true, 12, x"04B6", fail_count);
